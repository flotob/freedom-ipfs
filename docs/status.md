...
M10 interop hardening: partial. Unit tests, deterministic local Bitswap and light-DHT coverage, no-listener libp2p client swarm tests, Kubo-generated UnixFS/HAMT/range parity smoke, live ENS smoke, a checked-in public corpus covering immutable `/ipfs` and DNSLink-backed `/ipns` paths with live byte-range checks, a local cached-gateway RSS soak, and a host live-retrieval RSS soak exist, but larger media public corpus cases, broader Kubo parity matrix, and device network soaks remain follow-up work.
