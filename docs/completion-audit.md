...
Implementation audited code head: `6d15756a80aef9106ef5997ee593c3228d951538`; latest Apple-platform implementation/workflow CI evidence is recorded below by head SHA
...
```text
GitHub Actions iOS XCFramework run 25286090027 passed on 2026-05-03
Head SHA: 6d15756a80aef9106ef5997ee593c3228d951538
Job URL: https://github.com/flotob/freedom-ipfs/actions/runs/25286090027/job/74130655203
```
...
- `cargo fmt --all --check && make verify` passed on the audited head, including `xtask` coverage for the required Bee-on/Bee-off device evidence matrix and validation of the 19-row template.
...
- Public corpus smoke passed for the checked-in `vitalik-home`, `daicowtf-home`, `ipfs-tech-developers-hero`, `ipfs-tech-ribbon-community-7`, `ipfs-tech-ribbon-community-8`, `ipfs-tech-ribbon-home-1`, `ipfs-tech-ribbon-home-2`, `ipfs-tech-ribbon-home-3`, and `wikipedia-on-ipfs-en-root` entries, including byte-range requests for each entry:
  ...
  - `wikipedia-on-ipfs-en-root` returned `169` bytes from `/ipns/en.wikipedia-on-ipfs.org`.
  - The first eight range responses returned `128` bytes; the Wikipedia-on-IPFS range response returned `64` bytes. Every range response matched the full response prefix and included a valid `Content-Range` header.
  - Retrieval stats: `cache_hits=157 http_provider_blocks=8 bitswap_blocks=7`.
...
- Kubo parity passed for generated CIDv1/raw-leaf UnixFS site files, CIDv0/DAG-PB UnixFS site files, empty files, percent-encoded browser paths, directory-index fallback, no-index directory listings including HAMT listings, fixed/open-ended/suffix range reads, full/ranged `HEAD` requests, and HAMT directories.
...
| DNSLink support | DNSLink parser, pluggable TXT resolver trait, generic default resolver wrapper, Cloudflare DoH backend, split quoted TXT character-string concatenation, TTL-aware records, bounded CNAME delegation following, tests. | Done |
...
| Minimal Bitswap client | Deterministic in-process libp2p Bitswap tests verify retrieval, cache insert, want-have before want-block for multi-peer sessions, DONT_HAVE handling, cancel behavior, and no-listener client swarm construction; ignored loopback Kubo v0.41.0 daemon interop smoke verifies real Bitswap retrieval from Kubo. | Done for MVP |
...
| Routing modes | `auto`, `delegated`, `light_dht`, `offline` paths exist across CLI/mobile/gateway constructors; C ABI and Swift wrapper expose explicit online-gateway restart for Settings-style routing changes, including cache-only `.offline`; CLI process smoke launches the actual gateway binary with `--online --routing-mode offline`, imports a CAR, fetches it through `/ipfs/{cid}`, and proves an uncached CID does not hit a configured delegated router. | Done |
...
| Swift wrapper | `ffi/swift/FreedomIpfsReader.swift` exists with gateway start/stop/restart, routing-mode change including `.offline`, cache stats, retrieval/routing counters and deltas, combined diagnostics snapshot and per-navigation delta helper, active preload count, cache, lifecycle, preload/cancel, multi-router, and local URL mapping helpers; GitHub Actions Swift simulator smoke compiled and linked it against the XCFramework, checked non-loopback bind rejection, checked Swift-visible cached retrieval/routing counters, diagnostics, and diagnostics deltas, exercised lifecycle/low-memory/network-change/routing-restart/offline controls, and a generated UIKit/WebKit app rendered a local gateway fixture through `WKWebView`. | Done for simulator smoke; Freedom app integration unverified |
...
| Device/app verification runbook | `docs/ios-device-verification.md` defines app wiring, real-device matrix, Bee co-residency runs, live content cases, range checks, lifecycle checks, acceptance targets, and points to `docs/ios-device-evidence-template.csv` for structured measurement capture. `xtask validate-ios-device-evidence` validates the template, requires Bee-on and Bee-off rows for every IPFS-enabled case plus a Bee-only cold-idle baseline, and enforces stricter schema/resource-threshold checks for filled evidence with `--filled`. | Done; evidence still missing |
| Browser integration helpers | Local gateway URL, `ipfs://`/`ipns://`/gateway-style URL mapping helpers, preload/cancel with path/URI/bare-CID normalization, cache stats/control, retrieval/routing counters and deltas, combined diagnostics snapshot and delta helper, routing restart including `.offline`, and active preload count. | Swift compile/link, lifecycle/control calls, diagnostics deltas, routing restart/offline, and generated `WKWebView` app smoke verified in simulator; Freedom app integration unverified |
...
| Public IPFS/IPNS corpus | Checked-in opt-in corpus covers the ENS-derived immutable `/ipfs` paths plus DNSLink-backed `/ipns` paths for `ipfs.tech` assets and the Wikipedia-on-IPFS root; `make live-corpus` passed with full-response and byte-range checks. Mutable `/ipns/ipfs.tech`, `/ipns/dist.ipfs.tech`, `/ipns/cid.ipfs.tech`, and `/ipns/dnslink.dev` paths were retired from the default corpus after repeated public-provider `502` failures. | Expanded; should still diversify further |
| Kubo parity | Deterministic Kubo-generated CIDv1/raw-leaf UnixFS, CIDv0/DAG-PB UnixFS, empty-file, encoded-path, HAMT, no-index directory listing, fixed/open-ended/suffix range, full/ranged `HEAD`, and directory-index parity tests, plus loopback Kubo Bitswap interop. | Expanded |
...
3. Keep diversifying the public corpus beyond the ENS-derived paths, `ipfs.tech`, and Wikipedia-on-IPFS.
