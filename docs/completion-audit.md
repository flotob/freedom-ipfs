...
```text
GitHub Actions iOS XCFramework run 25270803278 passed on 2026-05-03
Head SHA: fd013d28f8d8882d3c7c767b69618f925a29b829
Job URL: https://github.com/flotob/freedom-ipfs/actions/runs/25270803278/job/74092706689
```
...
- GitHub Actions `iOS XCFramework` passed on `macos-15` with Xcode 16.4. It built the real `FreedomIpfs.xcframework`, verified headers/module maps/exported C symbols, booted an iOS simulator, compiled and linked the Swift wrapper smoke, started the local gateway in the simulator via `simctl spawn booted`, fetched the CAR fixture through loopback, stopped the gateway, built and installed a generated UIKit/WebKit simulator app, imported the same CAR fixture, started the gateway from app process, fetched `/ipfs/{cid}` through loopback with `URLSession`, rendered the HTML in `WKWebView`, verified the DOM marker with JavaScript, and uploaded `FreedomIpfs.xcframework` as artifact ID `6768364970` (`60010020` bytes).
...
| XCFramework verifier | `xtask verify-xcframework` checks slices, headers, module maps, exported symbols, a simulator Swift command-line gateway smoke, and a generated simulator app-rendering smoke on macOS; GitHub Actions run `25270803278` passed. | Done in macOS CI |
